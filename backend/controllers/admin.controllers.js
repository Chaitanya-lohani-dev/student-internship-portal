...
export const createJob = async (req, res) => {
  try {
    ...
    if (!validation.success) {
      return res
        .status(400)
        .json({ message: "Invalid Data", error: validation.error });
    }
...
export const getJobs = async (req, res) => {
  try {
    ...
    data = await Job.find({ createdBy: req.user.userId }).sort({
      createdAt: -1,
    });
    await client.set(`admin:jobs:${req.user.userId}`, JSON.stringify(data), {
      EX: 3600,
    });
...
export const getApplications = async (req, res) => {
  try {
    ...
    applications = await Application.find({ jobId: req.params.id }).sort({
      appliedAt: -1,
    });
    await client.set(
      `admin:applications:${req.params.id}`,
...
export const updateApplications = async (req, res) => {
  try {
    ...
    if (!application) {
      return res
        .status(400)
        .json({ message: "Invalid application or Not Found" });
    }
